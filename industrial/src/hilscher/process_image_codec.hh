...
/// Host-side encode/decode of a Hilscher process image.
/// Does not talk to hardware and does not invent fieldbus packets.
...
inline std::size_t processValueSize(ProcessValueType type)
{
  switch (type)
  {
    ...
    double *out)
{
  ...
  const std::size_t length = processValueSize(type);
  if (length == 0 || byteOffset + length > image.size())
  {
    return false;
  }
