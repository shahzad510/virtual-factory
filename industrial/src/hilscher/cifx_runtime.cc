...
bool CifxChannel::queryInfo(CifxChannelInfo *info, std::string *error)
{
  ...
  info->boardName.assign(channelInfo.abBoardName, strnlen(channelInfo.abBoardName, sizeof(channelInfo.abBoardName)));
  info->firmwareName.assign(
      reinterpret_cast<const char *>(channelInfo.abFWName),
      strnlen(reinterpret_cast<const char *>(channelInfo.abFWName), sizeof(channelInfo.abFWName)));
  ...
  if (xChannelIOInfo(
      ...
      == CIFX_NO_ERROR)
  {
    info->inputAreaBytes = ioInfo.ulTotalSize;
    info->outputAreaBytes = ioInfo.ulTotalSize;
  }
...
bool CifxDriver::acquire(std::string *error)
{
  if (error != nullptr)
  {
    *error = "Hilscher cifX SDK not compiled in (VF_HILSCHER_CIFX_AVAILABLE=0)";
  }
  return false;
}
...
bool CifxChannel::open(const std::string &, unsigned, std::string *error)
{
  if (error != nullptr)
  {
    *error = "Hilscher cifX SDK not compiled in";
  }
  return false;
}
