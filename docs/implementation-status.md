...
**ICP lifecycle isolation:** **IMPLEMENTED** / **TESTED** on `cursor/icp-lifecycle-isolation-a88d` (baseline M1.1 `71c2eb1`). Blocking connect/disconnect/reconnect I/O runs on `LifecycleExecutor` (per-adapter serial, cross-adapter parallel). Poll thread only observes/schedules/enqueues. HTTP Connect/Reconnect are async-accepted. Cross-adapter equipment collision/ownership uses a manager-level index (no peer `io_mutex` wait). `icp_lifecycle_isolation_test` proves mock + HTTP independence under blackhole OPC UA. Redundant Reconnect during in-flight Connect/RecoveryConnect does not bump generation or tear down a successful still-desired connect. Operator Connect/Reconnect during an in-flight connect-style job records a pending intent and enqueues one follow-up Connect if that job fails; rematerialize clears that pending intent (`icp_lifecycle_recovery_test`). A generation bump that drops a pending RecoveryConnect reconciles `autoReconnectInFlight` / `reconnect_in_progress_` so automatic recovery can continue (`icp_lifecycle_recovery_test` case H). Hung `poll()` holding `io_mutex` is fail-fast on `connectAdapter` (`ioBusy`) and, after 2s obstructed-poll detection, extract/rematerialize isolation so a replacement runtime can `RecoveryConnect` without cancelling the old poll (`icp_lifecycle_recovery_test` cases I–M). An obstructed automatic `RecoveryConnect` that holds `io_mutex` past the adapter `timeoutMs` plus slack is isolated the same way; a timeout-bound RecoveryConnect is not rematerialized and reaches FAULTED on the same runtime (`icp_lifecycle_recovery_test` cases N–O). Operator Connect/Reconnect (including coalesced operator Connect) is not isolated.
...
- `virtual_factory::icp::AdapterManager` — owns adapters; fail-fast `connectAdapter` (`ioBusy` if `io_mutex` is held); duplicate adapter id / equipment id collision checks; `forEachAdapterNonBlocking` for poll
- `virtual_factory::icp::PollScheduler` — one scheduler thread; `pollOnce()`; after-poll hook schedules recovery by **enqueue only**; obstructed poll rematerializes after 2s; obstructed automatic RecoveryConnect rematerializes after `timeoutMs` plus slack
- `virtual_factory::icp::PollExecutor` — parallel `poll()`; `hasInFlight(adapterId)` for isolation detection
